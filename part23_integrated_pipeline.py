"""Unified Part 2 / Part 3 video object removal pipeline.

This script is the single entry point for the improved SOTA pipeline and the
adaptive-refinement exploration:

- mode=part2: SAM2/YOLO-prompt masks + ProPainter/OpenCV inpainting.
- mode=part2_motion_strict: Part 2 with motion-filtered prompt generation.
- mode=part3_adaptive: raw SAM2 masks followed by adaptive motion-guided
  refinement, temporal identity support, and inpainting.

The goal is to keep Part 2 as a robust general baseline while making the Part 3
improvement reproducible from the same command-line interface.
"""
...
def copy_mask_sequence(src_dir: str, dst_dir: str) -> int:
    """Copy raw masks into the standard final-mask folder for non-refinement modes."""
    ensure_dir(dst_dir)
    count = 0
    for idx, src in enumerate(sorted_image_paths(src_dir)):
        dst = os.path.join(dst_dir, f"mask_{idx:05d}.png")
        shutil.copy2(src, dst)
        count += 1
    return count

def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser("Unified Part 2 / Part 3 Pipeline: SAM2 + ProPainter + Adaptive Refinement")
    ap.add_argument("--input", type=str, required=True)
    ap.add_argument("--output-dir", type=str, default="outputs/part23_run")
    ap.add_argument(
        "--mode",
        type=str,
        default="part2",
        choices=["part2", "part2_motion_strict", "part3_adaptive"],
        help="part2 keeps raw masks; part2_motion_strict uses motion-filtered prompts; part3_adaptive refines raw masks.",
    )
    ap.add_argument("--config", type=str, default=None, help="Optional YAML backend config. If omitted, a mode-specific config is used.")
    ...
    ap.add_argument("--output-prefix", type=str, default=None, help="Defaults to the selected --mode")
...
def main() -> None:
    ...
    if args.config is None:
        default_configs = {
            "part2": "part2_example.yaml",
            "part2_motion_strict": "part2_motion_strict.yaml",
            "part3_adaptive": "part3_adaptive_motion_refinement.yaml",
        }
        args.config = str(Path(project_root) / "configs" / default_configs[args.mode])

    if args.output_prefix is None:
        args.output_prefix = args.mode
    ...
    if args.mode in {"part2", "part2_motion_strict"}:
        refined_mask_count = copy_mask_sequence(raw_mask_dir, mask_dir)
        refinement_meta = {
            "enabled": False,
            "mode": args.mode,
            "note": "Raw Part 2 masks are copied as final masks; no adaptive refinement is applied.",
        }
    else:
